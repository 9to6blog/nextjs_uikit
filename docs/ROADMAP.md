...
목표는 여러 프로젝트에서 UI를 새로 만들 필요가 없는 독립 Next.js 라이브러리입니다. v0.1은 초기 69개 항목에 영상 기반 모션 패턴 10개를 추가해 현재 79개 문서 항목을 제공합니다. 아래 확장 항목과 기능 심화까지 완료하기 전에는 주요 UI 라이브러리 전체를 대체한다고 표현하지 않습니다.
